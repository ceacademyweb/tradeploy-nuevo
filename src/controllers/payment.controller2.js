    ...
    request.post(
        ...
        (err, response) => {
            ...
            Temo.findOneAndDelete({paypalId: pyID}, (err, resultTemo) => {
                ...
                Subscriptions.findOne({telegram: resultTemo.telegram}, (err, resultUser) => {
                    if (err) return res.status(500).json({'error':err, message: 'Error al buscar usuario'})
                    if (resultUser) {
                        ...
                        Subscriptions.findByIdAndUpdate(resultUser._id, updateSubs, (err, userUpdated) => {
                            if (err) return res.status(500).json({'error':err})
                            res.redirect(`${front}/suscripcion/renovacion/${resultUser._id}`)
